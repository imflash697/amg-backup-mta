	...
	if not isPlayerMapVisible() and getElementData(localPlayer, "loggedin") == 1 then
		...
		if isActive() then
			...
			if isUAT then
				...
				ax = ax - iconW
			elseif isAdmin then
				if getElementData( localPlayer,"duty_admin" )  == 1 and (isScripter and getElementData( localPlayer,"duty_dev" )  == 0 and isSM > 0)  then
					dxDrawImage(ax,ay,iconH,iconH,"images/hud/admin.png")
					table.insert(tooltips, "adminonduty")
									ax = ax - iconW
				elseif isSM > 0 then
					dxDrawImage(ax,ay,iconH,iconH,"images/hud/admin.png", 0, 0, 0, disabled_item(100))
					table.insert(tooltips, "adminoffduty")
									ax = ax - iconW
				end
			end
			...
			if (isGM or isAdmin) and (getElementData( localPlayer,"duty_admin" )  == 1) then
