...
function startStep11(quests1, retest, remainHours, remainMinutes, remainSeconds)
	...
	guiStep11.back = guiCreateButton ( bX, bY, bW, bH, "< قوانين سيرفر", false)
	guiStep11.next = guiCreateButton ( bX+bW, bY, bW, bH, "التالي >", false)
	guiStep11.retest = guiCreateButton ( bX+bW, bY, bW, bH, "اعادة الاختبار", false)
	...
	if text1 then 
		guiSetEnabled(guiStep11.next, false)
		guiSetText(guiStep11.intro, "للاسف لم يتم قبولك معنى في سيرفر"..text1.." اقراء القوانين جيداثم احد المحاولة")
...
function clickOnApplication()
	if source == guiStep11.next then
		...
		if incorrects > 0 then
			...
			guiSetText(guiStep11.intro, "تم اجتياز هذا الاختبار الان لننتقل للاختبار النهائي ")
...
function showServerRules()
	...
	guiRules.acceptRules = guiCreateButton ( 0.45, 0.81, 0.1, 0.04, "التالي", true)
